	...
	public class WorldRedirection : Interaction {
		...
		private void Update() {
			...
			if (techniqueInstance is not null && strategyInstance is not null) {
				...
				techniqueInstance.Redirect(forwardTarget: scene.forwardTarget, physicalHead: scene.physicalHead, virtualHead: scene.virtualHead, previousPosition: scene.previousPosition, previousOrientation: scene.previousRotation);
			}
