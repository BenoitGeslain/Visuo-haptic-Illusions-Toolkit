...
public class Utilisateur_AFP_Projet_Test : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("� cocher avant le lancement, affiche tous les objets tagg�s obstacles")]
    bool afficher_nom_et_position = false;
    [SerializeField]
    [Tooltip("Affiche la distance entre l'utilisateur et chaque obstacle en temps r�el")]
    ...
    // r�cup�ration de la position des objets uniquement au start 
    ...
    void Start()
    {
        ...
        if (afficher_nom_et_position)
        {
            ...
            {
                Debug.Log($"obstacle {i} : {Obstacles_gameObject[i].name} � la position : {Position_Obstacles[i]} ");
    ...
    void Update()
    {
        ...
        if (Position_utilisateur != LastPosition_utilisateur)
        {
            ...
            if (position_utilisateur) Debug.Log($"L'utilisateur est � la position {Position_utilisateur}");
    ...
    void CalculDistanceObstacles()
    {
            ...
            if (afficher_distance_objets)
            {
                ...
                {
                    Debug.Log($"Distance de l'objet {Obstacles_gameObject[i].name} � {Distance_Obstacles[i]} pixels");
                }
            }
