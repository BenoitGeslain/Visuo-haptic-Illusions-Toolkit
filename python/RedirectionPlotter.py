...
while True:
    ...
    with clientsocket:
        while True:
            ...
            ax1.stackplot(ys, otrsSum, rsSum, csSum, labels=('Over time\nrotation', 'Rotational', 'Curvature'), colors="rgb")
            ...
            for series, color, label in zip([otrs, rs, cs], "rgb", ["Over Time\nRotation", "Rotational", "Curvature"]):
                 ax2.plot(ys, series, color=color, label=label, linewidth=1)


            for ax in (ax1, ax2):
                ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), title="Redirection source")
            plt.pause(0.05)